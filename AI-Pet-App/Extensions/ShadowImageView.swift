...
public class ShadowImageView: UIView {
    ...
    public var blurredImageView = UIImageView()
...
//        }
    ...
    public func layoutWithViewSize(image:UIImage, width: CGFloat, height: CGFloat) {
        layoutImageView()
        self.frame = CGRect(origin: .zero, size: CGSize(width: width, height: height))
            // guard let image = self.image else {
            //     return
            // }
            // find what we need to scale the image to fit the view using shadowRadiusOffSetPercentage
            // let inverseScale = 1/(1.4 * (1 + self.shadowRadiusOffSetPercentage/100))
            let realImageSize = CGSize(width: width, height: height) // inverse scale
            DispatchQueue.global(qos: .userInteractive).async { [weak self] in
                guard let weakself = self else {
                    return
                }
