...
class LimitedPack {
    ...
    // fetch limited packs from firebase realtime database (NOT FIRESTORE)
    static func fetchLimitedPacks(completion: @escaping ([LimitedPack]) -> Void) {
        var limitedPacks = [LimitedPack]()
        let db = Database.database().reference()
        let docRef = db.child("limited_packs")
        // fetch limited packs and add to limitedPacks
        docRef.observeSingleEvent(of: .value, with: { (snapshot) in
        print("snapshot: \(snapshot)")
        print("number of limited packs: \(snapshot.childrenCount)")
        // queue so we can wait for all images to download
        let queue = DispatchQueue(label: "limitedPacks")
        queue.async {
                            ...
                            docRef.getDocument { (document, error) in
                                if let document = document, document.exists {
                                    ...
                                    if let featuredImage = featuredImage {
                                        let limitedPack = LimitedPack(title: title, subtitle: subtitle, description: description, featuredImage: featuredImage, price: price, internal_id: internal_id, demo_order: demo_order, expire_timestamp: expire_timestamp, banner_message: banner_message, index: data["index"] as? Int ?? 0)
                                        limitedPacks.append(limitedPack)
                                    } else {
                                    ...
                                    let banner_message = data["banner_message"] as? String
                                    if let featuredImage = featuredImage {
                                        let limitedPack = LimitedPack(title: title, subtitle: subtitle, description: description, featuredImage: featuredImage, price: price, internal_id: internal_id, demo_order: nil, expire_timestamp: expire_timestamp, banner_message: banner_message, index: data["index"] as? Int ?? 0)
                                        limitedPacks.append(limitedPack)
                                    } else {
                                        print("* looks like featured image isn't valid")
                                    }
        ...
        queue.sync {
            ...
            limitedPacks.sort(by: { $0.index < $1.index })
            // call completion handler
                completion(limitedPacks)
        }
