...
public class StepBar: UIStackView {
    ...
    @IBInspectable var stepCount: Int = 4
    @IBInspectable var currentStep: Int = 2
    ...
    @IBInspectable var progressTintColor: UIColor = Constants.primaryColor.hexToUiColor()
    @IBInspectable var stepTitleColor: UIColor = .gray
    @IBInspectable var stepSubTitleColor: UIColor = .lightGray
    @IBInspectable var stepSelectedTitleColor: UIColor = Constants.primaryColor.hexToUiColor()
    @IBInspectable var PulseColor: UIColor = .lightGray
    @IBInspectable var stepColor: UIColor = .gray
    @IBInspectable var CircleSize: CGFloat = 10
    @IBInspectable var SelectedCircleSize: CGFloat = 15
    @IBInspectable var mainFontSize: UIFont? = UIFont(name: "HelveticaNeue-Bold", size: 12)
    @IBInspectable var smallFontSize: UIFont? = UIFont(name: "HelveticaNeue", size: 10)
...
//                labelSub = UILabel(frame: CGRect(x: CGFloat(lblX) , y: CGFloat((stepButton.center.y + label.frame.height - 5) + (CircleSize/2)), width: lblWidth, height: 40))
            }
            ...
            if (stepSubTitles.count > 0 && stepSubTitles.count == stepCount) {
                if (stepSubTitles.count <= stepCount && ShowSubTitle) {
                    ...
                    labelSub.numberOfLines = 0 // vertical ? 1 : 0
                    labelSub.textAlignment = vertical ? .left : .center
                    labelSub.adjustsFontSizeToFitWidth = false
                    labelSub.minimumScaleFactor = 0.7
                    labelSub.font = smallFontSize
                }
            }
            ...
            stepButton.layer.borderColor = progressTintColor.cgColor
    ...
    deinit {
        // check if the observer is still registered
        if let _ = self.layer.presentation()?.value(forKeyPath: #keyPath(currentStepValue)) {
            self.layer.removeObserver(self, forKeyPath: #keyPath(currentStepValue))
        }
